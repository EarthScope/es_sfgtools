...
import yaml
from es_sfgtools.data_models.metadata.vessel import AtdOffset
from es_sfgtools.data_models.metadata.site import Site
from es_sfgtools.data_models.metadata.benchmark import Location, Benchmark, Transponder, TAT
...
def config_yaml_to_siteconfig(
    source: Union[str, Path], site_name: str, network: str
) -> Union[Site, None]:
    """
    Read the site configuration from a YAML file
    """
    if isinstance(source, str):
        source = Path(source)

    assert source.exists(), f"YAML file {source} does not exist"
    ...
    if "array_center" in data:
        site.arrayCenter = Location(
            ...
            longitude=data["array_center"].get(
                "lon",  None),
            elevation=None,  # Assuming no z-coordinate in the YAML file
        )

    # set benchmarks and transponders
    if "transponders" in data:
        for transponder_data in data["transponders"]:
            # transponder = Transponder(
            #     address=,
            #     tat=transponder_data["internal_delay"],
            #     start=,
            # )
            benchmark = Benchmark(
                name=transponder_data["pxp_id"],
                aPrioriLocation=Location(
                    latitude=transponder_data["lat"],
                    longitude=transponder_data["lon"],
                    elevation=transponder_data["height"],
                ),
            )
            site.benchmarks.append(benchmark)
    return site

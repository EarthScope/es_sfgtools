...
def merge_shotdata_kinposition_sfgdstf(
    shotdata_pre: TDBSFGDSTFSeafloorAcousticDataArray,
    shotdata: TDBSFGDSTFSeafloorAcousticDataArray,
    ...
    logger.loginfo("Merging sfgdstf shotdata and kin_position data")
    for date in dates:
        ...
        # Adapt the dataframe to the format expected by the main function
        shotdata_df_adapted = shotdata_df.rename(columns={
            'T_transmit': 'pingTime',
            'T_receive': 'returnTime',
            'X_transmit': 'east0',
            'Y_transmit': 'north0',
            'Z_transmit': 'up0',
            'X_receive': 'east1',
        ...
        shotdata_df_adapted['isUpdated'] = False

        # interpolate the enu values
        shotdata_df_updated = main(
            shotdata=shotdata_df_adapted,
        ...
        # Adapt the dataframe back to the SFGDSTF format
        shotdata_df_final = shotdata_df_updated.rename(columns={
            'pingTime': 'T_transmit',
            'returnTime': 'T_receive',
            'east0': 'X_transmit',
            'north0': 'Y_transmit',
            'up0': 'Z_transmit',
            'east1': 'X_receive',
        ...
        # Drop the isUpdated column as it is not part of the schema
        shotdata_df_final = shotdata_df_final.drop(columns=['isUpdated'])


        shotdata.write_df(shotdata_df_final, validate=False)

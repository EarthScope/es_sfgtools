...
# # Local imports
# from ..data_models.constants import STATION_OFFSETS,TRIGGER_DELAY_SV2
# from ..data_models.log_models import PositionData, RangeData,BestGNSSPOSDATA
...
#     tt_array: np.ndarray = get_traveltime(
#         df["range"].to_numpy(), df["tat"].to_numpy(), triggerDelay=TRIGGER_DELAY_SV2
#     )
#     df["tt"] = tt_array
...
#                 logger.logerr(
#                     f"Acoustic Parsing:{e} | Error parsing FILE {source} at LINE {line_number}"
#                 )
#                 pass
...
#                 try:
#                     range_data: List[RangeData] = RangeData.from_sv2(
#                         line, main_offset_dict
#                     )
#                     simultaneous_interrogation_set.extend(range_data)
#                 except ValidationError as e:
#                     logger.logerr(
#                         f"Error parsing into SimultaneousInterrogation from line {line_number} in {source}\n "
#                     )
#                     pass
...
#     logger.loginfo(
#         f"Acoustic Parser: {acoustic_df.shape[0]} shots from FILE {source.local_path} | {len(unique_transponders)} transponders | {shot_count} shots per transponder"
#     )
...
#                         position_data.sdx, position_data.sdy, position_data.sdz = (
#                             gnssMeta.sdx,
#                             gnssMeta.sdy,
#                             gnssMeta.sdz,
#                         )
...
#                         logger.logerr(
#                             f"IMU Parsing: An error occurred while parsing INVSPA data from FILE {source} at LINE {line_number} \n Error: {line}"
#                         )
#                         pass

#             except UnicodeDecodeError as e:
#                 logger.logerr(
#                     f"Position Parsing:{e} | Error parsing FILE {source.local_path} at LINE {line_number}"
#                 )
#                 pass
...
# def dev_merge_to_shotdata(
#     acoustic: DataFrame[AcousticDataFrame],
#     position: DataFrame[PositionDataFrame],
#     **kwargs,
# ) -> DataFrame[ShotDataFrame]:
#     """
...
#         lambda x: (x.replace(hour=0, minute=0, second=0, microsecond=0)).timestamp()
#     )
...
#         output_df[field] = interp(
#             acoustic_ping_dt.to_numpy(),
#             position["time"].apply(lambda x: x.timestamp()).to_numpy(),
#             position[target].to_numpy(),
#         )

#     for field, target in return_keys_position.items():
#         output_df[field] = interp(
#             acoustic_return_dt.to_numpy(),
#             position["time"].apply(lambda x: x.timestamp()).to_numpy(),
#             position[target].to_numpy(),
#         )
...
#     output_df = (
#         output_df.loc[:, ~output_df.columns.str.contains("^unnamed")]
#         .drop(columns=["time"])
#         .dropna()

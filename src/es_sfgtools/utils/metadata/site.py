...
sub_level_groups = [SURVEYS, TRANSPONDERS]
...
class ArrayCenter(BaseModel, AttributeUpdater):
    x: Optional[float] = Field(default=None, description="The x coordinate of the array center")
    y: Optional[float] = Field(default=None, description="The y coordinate of the array center")
    z: Optional[float] = Field(default=None, description="The z coordinate of the array center")
...
class Site(BaseModel):
    ...
    timeOrigin: datetime = Field(..., 
                                 ...
                                 ge=datetime(1901, 1, 1))
    localGeoidHeight: float = Field(..., description="The local geoid height of the site")

    # Optional
    arrayCenter: Optional[ArrayCenter] = Field(default_factory=dict, description="The array center of the site")
    campaigns: List[Campaign] = Field(default_factory=list, description="The campaigns associated with the site")
    benchmarks: List[Benchmark] = Field(default_factory=list, description="The benchmarks associated with the site")
    referenceFrames: List[ReferenceFrame] = Field(default_factory=list, description="The reference frames used for the site")
    ...
    top_level_map_components: ClassVar[Dict[str, Any]] = {
        REFERENCE_FRAMES: (lambda self: self.referenceFrames, ReferenceFrame),
        BENCHMARKS: (lambda self: self.benchmarks, Benchmark),
        CAMPAIGNS: (lambda self: self.campaigns, Campaign),
    }
    ...
    def run_component(
        ...
        if not component_metadata['name']:
            print("ERROR: Equipment name not provided, please provide an equipment ID to add, update or delete..")
            return
        
        if component_type not in top_level_groups:
            print("ERROR: Equipment type not recognised, please provide a valid equipment type..")
            return
    ...
    def _new_component(self, component_type: str, component_metadata: dict):
        """
    ...
    def _update_existing_component(self, component_type: str, component_metadata: dict):
        """
    ...
    def run_sub_component(
        ...
        if component_type not in top_level_groups:
            print(f"ERROR: {component_type} not recognised, please provide a valid component type..")
            return
        
        
        if sub_component_type == TRANSPONDERS:
        ...
        if add_new:
            self._new_sub_component(component_type, component_name, sub_component_type, sub_component_metadata)
        if update:
            self._update_existing_sub_component(component_type, component_name, sub_component_type, sub_component_metadata)
        if delete:
            self._delete_sub_component(component_type, component_name, sub_component_type, sub_component_metadata['name'])

    def _new_sub_component(self, component_type: str, component_name: str, sub_component_type: str, sub_component_metadata: dict):
        """
            ...
            if equipment.name == component_name:
                if sub_component_type == TRANSPONDERS:
                    ...
                    try:   
                        ...
                        return
                    equipment.surveys.append(new_survey)
                    print(new_survey.model_dump_json(indent=2))
                    print(f"New survey added successfully.")
                    return

    def _update_existing_sub_component(self, component_type: str, component_name: str, sub_component_type: str, sub_component_metadata: dict):
        """
                    ...
                    for transponder in equipment.transponders:
                        if transponder.address == sub_component_metadata['address']:
                            ...
                            if 'extraSensors' in sub_component_metadata:
                                transponder.extraSensors.append(sub_component_metadata['extraSensors'])
                                print(f"Added sensor to transponder {transponder.address}.")

                            elif 'batteryVoltage' in sub_component_metadata:
                                transponder.batteryVoltage.append(sub_component_metadata['batteryVoltage'])
                                print(f"Added battery voltage to transponder {transponder.address}.")
    ...
    def _delete_sub_component(self, component_type: str, component_name: str, sub_component_type: str, sub_component_name: str):

...
class StationData(BaseModel):
    ...
    gnssdata: str = Field(default=None, description="The station's RINEX derived position TileDB URI")
    gnssobsdata: str = Field(default=None, description="The station's raw gnss observables TileDB URI")
    positiondata: str = Field(default=None, description="The station's position data TileDB URI")
...
class Catalog(BaseModel):
    ...
    @classmethod
    def load_data(cls,path:str|Path|dict,name=None,info=None) -> 'Catalog':
    ...
    @classmethod
    def load_metadata(cls,data_path:Path,name=None,info=None) -> 'Catalog':
        # load vessels
        vessels = {}
        ...
        soundSpeedProfiles = {}
        for ssp in (data_path/"sound_speed").iterdir():
            soundSpeedProfiles[ssp.stem] = pd.read_csv(ssp, sep=",", header=0, index_col=0)
        ...
        for network in (data_path / "networks").iterdir():
            ...
            for station in network.iterdir():
                ...
                for campaign in built_site.campaigns:
                    # load vessel and sound speed profile
                    campaign.vessel = vessels[campaign.vesselCode]
                    if getattr(campaign,"soundSpeedCode") != None:
                        campaign.soundSpeedProfile = soundSpeedProfiles[campaign.soundSpeedCode]
